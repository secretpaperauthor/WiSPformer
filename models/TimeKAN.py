...
class ConcatFusion(nn.Module):
    def __init__(self, enc_in, seq_in, use_gate=True, dropout=0.0):
        super().__init__()
        self.use_gate = use_gate
        if use_gate:
            self.gate = nn.Parameter(torch.ones(1, 1, enc_in) * 0.5)

        self.fuse = nn.Sequential(
            nn.Linear(seq_in * 2, 512),
            nn.GELU(),
            nn.Linear(512, seq_in),
        )
        self.dropout = nn.Dropout(dropout)

    def forward(self, x, Zf, use_concat=True):
        x_spec = torch.fft.irfft(Zf, n=x.shape[1], dim=-1, norm="ortho")  # [B, C, T]
        x_spec = x_spec.permute(0, 2, 1).contiguous()   # [B, T, C]

        if use_concat:
            # [B, Tx2, C] => [B, C, Tx2]
            x_spec = torch.cat([x, x_spec], dim=1)
            x_spec = self.fuse(x_spec.transpose(-1, -2)).transpose(-1, -2)
        
        elif self.use_gate: 
            g = torch.sigmoid(self.gate)
            x_spec = g * x + (1 - g) * x_spec
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.down_sampling_window = self.configs.down_sampling_window
        self.channel_independence = self.configs.channel_independence

        self.res_blocks = nn.ModuleList([FrequencyDecomp(self.configs)
                                         for _ in range(self.configs.e_layers)])
        self.add_blocks = nn.ModuleList([FrequencyMixing(self.configs)
                                         for _ in range(self.configs.e_layers)])

        self.preprocess = series_decomp(self.configs.moving_avg)
        self.enc_in = self.configs.enc_in
        self.use_future_temporal_feature = self.configs.use_future_temporal_feature


        self.enc_embedding = DataEmbedding_wo_pos(1, self.configs.d_model, self.configs.embed, self.configs.freq,
                                                      self.configs.dropout)
        self.layer = self.configs.e_layers
        self.normalize_layers = torch.nn.ModuleList(
            [
                Normalize(self.configs.enc_in, affine=True, non_norm=True if self.configs.use_norm == 0 else False)
                for i in range(self.configs.down_sampling_layers + 1)
            ]
        )
        self.projection_layer = nn.Linear(
                    self.configs.d_model, 1, bias=True)
